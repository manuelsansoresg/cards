...
@section('content')
<h4>Editar tarjeta #{{ $upload->id }}</h4>

<form method="POST" action="{{ route('admin.uploads.update', $upload) }}" class="mt-3" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="row g-3">
        <div class="col-md-6">
            <label class="form-label">Título</label>
            <input type="text" name="title" class="form-control" value="{{ old('title', $upload->title) }}" required>
        </div>
        <div class="col-md-3">
            <label class="form-label">Precio (USD)</label>
            <div class="input-group">
                <span class="input-group-text">$</span>
                <input type="number" step="0.01" name="price" id="price" class="form-control" value="{{ old('price', $upload->price) }}">
            </div>
        </div>
        <div class="col-md-3">
            <label class="form-label">Costo en stars</label>
            <div id="starsCostDisplay" class="form-control bg-light" style="height:38px; display:flex; align-items:center;">{{ old('stars_cost', $upload->stars_cost) }}</div>
            <input type="hidden" name="stars_cost" id="stars_cost" value="{{ old('stars_cost', $upload->stars_cost) }}">
            <small class="text-muted">1 USD equivale a {{ optional(\App\Models\StarsSetting::first())->stars_per_dollar ?? 1 }} stars</small>
        </div>
        <div class="col-md-3">
            <div class="form-check mt-4">
                <input class="form-check-input" type="checkbox" name="is_free" id="is_free" {{ $upload->is_free ? 'checked' : '' }}>
        ...
        <div class="col-md-6">
            ...
            <select name="categoria_id" class="form-select">
                ...
                @foreach($categorias as $c)
                    <option value="{{ $c->id }}" {{ $upload->categoria_id == $c->id ? 'selected' : '' }}>{{ $c->nombre }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-12">
...
<div class="row g-3">
    ...
    <div class="col-md-3">
        <div class="card">
            @if($m->file_type==='image')
                <img src="/{{ $m->file_path }}" class="card-img-top" alt="media">
            @else
                <div class="p-3">{{ $m->file_type }}<br><small>{{ $m->file_path }}</small></div>
            @endif
            <div class="card-body text-center">
                <form action="{{ route('admin.uploads.media.destroy', $m) }}" method="POST" class="delete-form">
                    @csrf
                    @method('DELETE')
    ...
    @endforeach
    @if($media->isEmpty())
        <div class="col-12"><em>No hay archivos subidos aún.</em></div>
    @endif
 </div>
@endsection